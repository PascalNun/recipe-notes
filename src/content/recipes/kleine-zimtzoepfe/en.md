---
order: 3
recipeKey: small-cinnamon-braids
lang: en
title: Small Cinnamon Braids
titleLines:
  - Small Cinnamon
  - Braids
subtitle: Airy yeast dough with cinnamon filling
slug: small-cinnamon-braids
description: Small plant-based cinnamon braids made from soft yeast dough with agave syrup, cinnamon and a tender crumb.
tags:
  - Yeast pastry
  - Plant-based
  - Cinnamon
yieldText: 14 small cinnamon braids
timeItems:
  - label: Preparation & shaping
    value: about 25–30 active minutes
  - label: Proofing
    value: usually about 75–110 minutes in total
  - label: Baking
    value: about 15–20 minutes
  - label: Total time
    value: approximately 2 hours
timeNote: The proofing times are guides. The dough should first roughly double in volume, and the shaped braids should look visibly risen and more relaxed before baking.
heroImage: ./assets/hero-cutout-v4.webp
heroAlt: Small golden vegan cinnamon braids with visible cinnamon layers in a contemporary food-zine composition
introNote: An uncomplicated plant-based yeast pastry with a tender crumb, a clear cinnamon note and a small graphic shape.
ingredientSpotImage: ./assets/ingredient-cinnamon.webp
ingredientSpotAlt: Cinnamon sticks, ground cinnamon and agave syrup in a small ingredient composition
ingredientSections:
  - title: For the yeast dough
    items:
      - 600 g flour
      - 250 ml oat milk
      - 60 g neutral oil
      - 30 g sugar
      - a little liquid stevia
      - 1 sachet dried yeast
      - 1 tsp salt
      - optional vanilla
      - optional cardamom
  - title: For the filling
    items:
      - about 2–3 tsp cinnamon, depending on the preferred intensity
      - 2–4 tbsp agave syrup
      - 1–2 tbsp sugar
      - optional splash of oat milk
      - optional small splash of oil
steps:
  - number: 1
    title: Make the yeast dough
    text: Warm the oat milk gently and stir in the sugar, stevia and dried yeast. Place the flour and salt in a large bowl, add the liquid and oil, then knead for several minutes to form a smooth dough.
    note: Cover and leave to rise, usually for about 60–90 minutes, until the dough has roughly doubled in volume.
    image: ./assets/step-01.webp
    imageAlt: Smooth risen yeast dough in a speckled ceramic bowl
  - number: 2
    title: Shape the braids
    text: Roll the risen dough into a rectangle and brush the surface evenly with agave syrup. Distribute the cinnamon and sugar evenly over it. Divide the prepared dough into 14 pieces of roughly equal size. Cut or divide each piece lengthways into three strands, leaving the strands connected at the top. Plait the three strands into a small braid, pinch the end gently and tuck it slightly underneath if needed. Place the braids on a baking tray.
    note: Leave to rise again, usually for about 15–20 minutes, until the braids look visibly puffed and more relaxed.
    imageAlt: Four images show how to fill, divide and plait one small three-strand braid
    sequenceImages:
      first: ./assets/step-02-stage-1.webp
      second: ./assets/step-02-stage-2.webp
      third: ./assets/step-02-stage-3-v2.webp
      fourth: ./assets/step-02-stage-4.webp
  - number: 3
    title: Bake
    text: Bake in a fan oven at 175–180 °C for about 15–20 minutes. The braids should be lightly golden and remain tender.
    image: ./assets/step-03.webp
    imageAlt: Six lightly golden elongated cinnamon braids on a dark baking tray
characterNotes:
  - tender crumb
  - warm cinnamon
  - restrained sweetness
  - optional cardamom and vanilla
contextNotes:
  - plant-based yeast pastry
  - contemporary café patisserie
  - practical individual portions
storage: Small portions freeze well and can be enjoyed one at a time.
nutrition:
  basis: Estimated values for 14 cinnamon braids. The midpoint of each stated range was used for cinnamon, agave syrup and sugar.
  totalLabel: Whole recipe
  servingLabel: Per piece
  items:
    - label: Energy
      total: 13,600 kJ / 3,250 kcal
      perServing: 970 kJ / 232 kcal
    - label: Fat
      total: 72.6 g
      perServing: 5.2 g
    - label: of which saturates
      total: 5.9 g
      perServing: 0.4 g
    - label: Carbohydrate
      total: 576.3 g
      perServing: 41.2 g
    - label: of which sugars
      total: 98.4 g
      perServing: 7.0 g
    - label: Fibre
      total: 22.9 g
      perServing: 1.6 g
    - label: Protein
      total: 66.8 g
      perServing: 4.8 g
    - label: Salt
      total: 6.3 g
      perServing: 0.45 g
  note: Optional vanilla, cardamom, extra oat milk and extra oil are not included.
tips:
  - A lower level of sweetness works well here.
  - Oil instead of margarine keeps the crumb soft.
---
