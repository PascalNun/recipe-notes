---
order: 2
recipeKey: hessian-kreppel
lang: en
title: Hessian Kreppel
titleLines:
  - Hessian
  - Kreppel
subtitle: Airy yeast dough, fried until golden
slug: hessian-kreppel
description: Light vegan Kreppel made from soft yeast dough, shaped individually and fried until golden.
tags:
  - Yeast pastry
  - Plant-based
  - Hessian roots
yieldText: about 15–20 pieces
timeItems:
  - label: Preparation
    value: about 20 minutes
  - label: First rise
    value: usually about 60–90 minutes
  - label: Frying
    value: about 2–3 minutes per side
  - label: Total time
    value: approximately 2 hours
timeNote: The proofing time is a guide. Leave the covered dough to rise until it has roughly doubled in volume. Room and dough temperature may make this faster or slower.
heroImage: ./assets/hero-cutout-v1.webp
heroAlt: Golden fried vegan Kreppel with an airy crumb in a contemporary food-zine composition
introNote: A plant-based interpretation of Hessian Kreppel – regionally rooted, made by hand and presented in a contemporary way.
ingredientSpotImage: ./assets/ingredient-yeast.webp
ingredientSpotAlt: Oat milk in a glass jug beside dried yeast and flour
ingredientSections:
  - title: For the yeast dough
    items:
      - 600 g flour
      - 125 g margarine
      - 70 g sugar
      - 1 sachet dried yeast or 42 g fresh yeast
      - 250 ml oat milk, lukewarm
      - 1 pinch salt
  - title: Also needed
    items:
      - oil for deep-frying
steps:
  - number: 1
    title: Activate the yeast
    text: Stir the yeast into the lukewarm oat milk with one teaspoon of the sugar.
    note: Leave to stand for about 10 minutes.
    image: ./assets/step-01.webp
    imageAlt: Activated yeast with fine bubbles in lukewarm oat milk
  - number: 2
    title: Make the dough
    text: Knead all the ingredients into a smooth yeast dough. Cover and leave to rise until it has roughly doubled in volume.
    note: The first rise usually takes about 60–90 minutes. The visible state of the dough matters more than an exact time.
    image: ./assets/step-02.webp
    imageAlt: Smooth risen yeast dough in a speckled ceramic bowl
  - number: 3
    title: Shape the Kreppel
    text: Roll the dough to a thickness of about 2 cm and cut out rounds.
    image: ./assets/step-03.webp
    imageAlt: Yeast dough rolled about two centimetres thick with cut rounds, a cutter and rolling pin
  - number: 4
    title: Fry
    text: Heat the oil to about 170–180 °C. Fry the Kreppel for 2–3 minutes on each side until golden, then drain them on kitchen paper.
    image: ./assets/step-04.webp
    imageAlt: Three Kreppel frying in a dark pot while small bubbles rise around a wooden skewer
characterNotes:
  - airy yeast dough
  - golden crust
  - gentle sweetness
contextNotes:
  - Hessian yeast pastry
  - plant-based interpretation
  - contemporary café culture
nutrition:
  basis: Estimated values based on 18 Kreppel and an average of 7.5 g absorbed frying oil per piece.
  totalLabel: Whole recipe
  servingLabel: Per piece
  items:
    - label: Energy
      total: 19,580 kJ / 4,680 kcal
      perServing: 1,090 kJ / 260 kcal
    - label: Fat
      total: 248.1 g
      perServing: 13.8 g
    - label: of which saturates
      total: 30.4 g
      perServing: 1.7 g
    - label: Carbohydrate
      total: 545.0 g
      perServing: 30.3 g
    - label: of which sugars
      total: 76.6 g
      perServing: 4.3 g
    - label: Fibre
      total: 19.3 g
      perServing: 1.1 g
    - label: Protein
      total: 66.7 g
      perServing: 3.7 g
    - label: Salt
      total: 3.1 g
      perServing: 0.17 g
  note: Actual values vary especially with the final yield, the margarine and the amount of frying oil absorbed.
tips:
  - When small bubbles rise around a wooden skewer dipped into the oil, the oil is hot enough.
---
