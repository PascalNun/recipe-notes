---
recipeKey: white-bean-cinnamon-cream
lang: en
title: White Bean Cinnamon Cream
titleLines:
  - "White Bean"
  - "Cinnamon Cream"
subtitle: with oat crunch & espresso
slug: white-bean-cinnamon-cream
description: A modern plant-based dessert built around a silky white bean cream, toasted sunflower seed butter, crisp oats and a touch of espresso.
tags:
  - Dessert
  - Plant-based
  - Regionally inspired
heroImage: ./assets/hero-cutout-v3.webp
heroAlt: A layered dessert of pale white bean cinnamon cream and oat crunch beside a small cup of espresso
ingredientSpotImage: ./assets/ingredient-sunflower-v2.webp
ingredientSpotAlt: Slender illustrated sunflower with loose sunflower seeds
ingredientEspressoImage: ./assets/ingredient-espresso.webp
ingredientEspressoAlt: Small speckled espresso cup with drawn steam marks
characterSpotImage: ./assets/character-vignette.webp
characterSpotAlt: Small composition of cinnamon cream, oat crunch, lime and cinnamon sticks
healthSpotImage: ./assets/health-botanical.webp
healthSpotAlt: Botanical illustration combining sunflower, bean plant and oat stems
claim: Regionally inspired. Modern by nature. Simply good.
introNote: A dessert made with ingredients that can plausibly be grown in Central Europe and approached from a regional perspective, then combined in a modern, international way. Contemporary, urban and distinctive rather than nostalgic.
ingredientSections:
  - title: For the cream
    items:
      - 1 can white beans, about 240 g drained
      - plain white beans packed in water or brine
      - 150–200 g vegan skyr or soy yoghurt
      - 2 tbsp agave syrup
      - 2 tbsp sugar
      - a few drops liquid stevia
      - 1 tsp vanilla
      - 1 tsp cinnamon
      - generous pinch of salt
      - 1–2 tsp lime juice
      - optional splash of plant milk for blending
  - title: For the sunflower seed butter
    items:
      - 80–100 g sunflower seeds
      - optional 1 tsp neutral oil
  - title: For the oat crunch
    items:
      - 60 g fine rolled oats
      - 35–40 g jumbo rolled oats
      - 15–20 g flour
      - 30 g sugar
      - 1–2 tsp agave syrup
      - 35–40 g neutral oil
      - 1 tsp cinnamon
      - small pinch of salt
      - 1/2 tsp baking powder
  - title: For the espresso finish
    items:
      - 1 small espresso
steps:
  - number: 1
    title: Toast the sunflower seeds
    text: Gently toast the sunflower seeds in a dry pan or in the oven until they smell warm, nutty and lightly caramelised. Do not let them get too dark. Blend while still warm until creamy, adding the smallest amount of oil only if needed.
    image: ./assets/step-01.webp
    imageAlt: Toasted sunflower seeds in a dark pan beside a blender containing smooth sunflower seed butter
  - number: 2
    title: Prepare the beans
    text: Rinse the beans very thoroughly and leave them to drain briefly. For an even more neutral flavour, you can rinse them once with hot water.
    image: ./assets/step-02.webp
    imageAlt: Rinsed white beans draining in a stainless-steel colander
    note: This makes them milder, creamier and less bean-like.
  - number: 3
    title: Make the cream
    text: Blend the beans, sunflower seed butter and a small splash of plant milk until as smooth as possible. Add the vegan skyr or yoghurt, agave, sugar, stevia, vanilla, cinnamon, salt and lime juice, then blend again until very fine.
    image: ./assets/step-03.webp
    imageAlt: Countertop blender filled with smooth pale white bean cinnamon cream
    note: Chill for 20–30 minutes. The cream will become a little firmer.
  - number: 4
    title: Prepare the oat crunch
    text: Mix all the ingredients together. The mixture should feel lightly moist and remain a little crumbly.
    image: ./assets/step-04.webp
    imageAlt: Bowl of moist oat crunch mixture with a spoon
  - number: 5
    title: Bake the crunch
    text: Spread the mixture fairly thinly and unevenly on baking paper. Bake at 175 °C conventional heat for about 18–25 minutes. Turn and loosen it once during baking.
    image: ./assets/step-05.webp
    imageAlt: Golden oat crunch on a dark baking tray beside a small oven
    note: Leave it to cool completely.
  - number: 6
    title: Add the espresso finish
    text: Lightly drizzle the crunch with espresso.
    image: ./assets/step-06.webp
    imageAlt: Espresso dripping from a spoon onto oat crunch in a bowl
    note: Do not soak it – the aim is simply to add a little aroma.
  - number: 7
    title: Layer the dessert
    text: Start with a little crunch, add some cream, follow with a loose layer of crunch, then spoon the remaining cream on top.
    imageAlt: Four glasses showing the dessert layered with crunch and cream in sequence
    sequenceImages:
      first: ./assets/step-07-stage-1.webp
      second: ./assets/step-07-stage-2.webp
      third: ./assets/step-07-stage-3.webp
      fourth: ./assets/step-07-stage-4.webp
finish:
  - a little cinnamon
  - a few pieces of oat crunch
  - optional toasted sunflower seeds
  - optional minimal drizzle of espresso on top
characterNotes:
  - toasted notes
  - oats
  - gentle bitterness
  - creamy texture
  - fresh lime
  - restrained sweetness
contextNotes:
  - café dessert
  - Nordic pastry
  - contemporary regional cooking
  - urban food culture
healthNotes:
  - White beans provide plant protein and fibre.
  - Sunflower seeds contain unsaturated fats, vitamin E and minerals.
  - Oats grown in Central Europe are regionally plausible and nutrient-rich.
---

The complete recipe is structured in frontmatter so ingredients, steps and notes stay easy to edit without a CMS.
